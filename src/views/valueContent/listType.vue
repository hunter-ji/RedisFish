...
const state: { search: string, values: listTableValueType[], multipleSelection: listTableValueType[] } = reactive({
  search: '',
  values: [],
  multipleSelection: []
})
...
const data: ComputedRef<listTableValueType[]> = computed(() => {
  if (state.search) {
    return state.values.filter((item: listTableValueType) => {
